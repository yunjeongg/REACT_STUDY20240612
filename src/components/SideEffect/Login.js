...
const emailReducer = (state, action) => {
  ...
  if (action.type === 'USER_INPUT') { // 체인지이벤트에서
    return { 
      inputValue: action.val,
      isVaild: action.val.includes('@') 
    };
  } else if (action.type === 'VALIDATE') { // 블러이벤트에서
    return { 
...
const Login = ({ onLogin }) => {
  ...
  /* 
    ...
    param1: 위에서 만든 Reducer 함수
    param2: 상태값의 초기값
    return: Reducer 를 관리하는 배열
          [0]: 이메일 관련 상태값
          [1]: 상태를 변경할 수 있는 함수
  */
  const [emailState, dispatchEmail] = useReducer(emailReducer, {
    inputValue: '', // 이메일 입력시
    isValid: null // 조건부합여부에따라 true 또는 false
  });
  // console.log('abc: ', abc);
  console.log('변경후 이메일상태: ', emailState);
